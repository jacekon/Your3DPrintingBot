"""
Test script for ElegooSlicer integration.
Run with: python -m tests.test_elegoo_slicer

This script tests the ElegooSlicer module independently before bot integration.
"""
import asyncio
import logging
from pathlib import Path

from src.slicer import ElegooSlicer
...
async def test_elegoo_slicer():
    """Test ElegooSlicer with sample STL files."""
    
    logger.info("=" * 60)
    logger.info("ElegooSlicer Integration Test")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info(f"   Binary: {slicer.elegoo_bin}")
    ...
    output_dir = Path(__file__).resolve().parent.parent / "data" / "test_gcode_elegoo"
    ...
    try:
        ...
        logger.info("\nTroubleshooting:")
        logger.info("1. Verify ElegooSlicer is installed")
        logger.info("2. Import your printer config in ElegooSlicer GUI:")
        logger.info("   File → Import → Import Configs...")
        logger.info("   Select: config/printers/Elegoo Centauri Carbon 0.4 nozzle.elegoo_printer")
        logger.info("3. Configure filament and print settings as defaults")
        logger.info("4. Try running ElegooSlicer manually to verify it works")
...
async def test_batch_slicing():
    ...
    output_dir = Path(__file__).resolve().parent.parent / "data" / "test_gcode_elegoo_batch"
    output_dir.mkdir(parents=True, exist_ok=True)
...
def main():
    """Run all tests."""
    asyncio.run(test_elegoo_slicer())
